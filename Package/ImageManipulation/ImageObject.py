...
#Provide functions for extracting a path of coordinates from a simplistic monochrome image.
...
class ImageObject : 
    ...
    def monochrome(self, scale = 3, threshold = 200) :
        ...
        #scale image down to reduce number of non_zero pixels
    ...
    def show_bw(self) : 
        print("Dimension:{}".format(self.bw_img.size))
    ...
    def get_tour(self, starting_point = "random", plot = True) : 
        ...
        absolute_index = np.where(self.pixels_flat > 0)[0] #Position of non-zero pixels
    ...
    def get_splines(self, degree = 1, plot = True) : 
        ...
        #Convert ours int parametric spline curves
    ...
    def plot_parametric(self, num_points = 1000) : 
        ...
        t_vals = zip(*[(v, self.x_spl(v)) for v in np.linspace(0, self.num_pixels, num_points)])
        x_vals = zip(*[(v, self.x_spl(v)) for v in np.linspace(0, self.num_pixels, num_points)])
